...
#define ROUND_SIZE(addr,size,mask) (((SIZE_T)(size) + ((UINT_PTR)(addr) & (mask)) + (mask)) & ~(mask))
...
static NTSTATUS set_protection( struct file_view *view, void *base, SIZE_T size, ULONG protect )
{
    ...
    else
    {
        ...
        if ((access & VPROT_WRITE) && (view->protect & VPROT_WRITECOPY))
            access &= ~VPROT_WRITE;
                          ...
                          ULONG_PTR limit_low, ULONG_PTR limit_high, size_t align_mask )
{
    ...
    else
    {
        ...
        if (!(ptr = alloc_free_area( (void *)limit_low, (void *)limit_high, host_size, top_down, get_unix_prot( vprot ), align_mask )))
        {
            WARN("Allocation failed, clearing native views.\n");

            clear_native_views();
            if (!is_win64) increase_try_map_step = FALSE;
            ptr = alloc_free_area( (void *)limit_low, (void *)limit_high, host_size, top_down, get_unix_prot( vprot ), align_mask );
                                     ...
                                     int shared_fd, BOOL removable )
{
    ...
    for (i = pos = 0; i < nt->FileHeader.NumberOfSections; i++, sec++)
    {
        ...
        if (!sec->Misc.VirtualSize)
            map_size = ROUND_SIZE( 0, sec->SizeOfRawData, page_mask );
        else
            map_size = ROUND_SIZE( 0, sec->Misc.VirtualSize, page_mask );
        ...
        /* a few sanity checks */
        end = sec->VirtualAddress + ROUND_SIZE( sec->VirtualAddress, map_size, page_mask );
    ...
    for (i = 0; i < nt->FileHeader.NumberOfSections; i++, sec++)
    {
        ...
        if (sec->Misc.VirtualSize)
            size = ROUND_SIZE( sec->VirtualAddress, sec->Misc.VirtualSize, page_mask );
        else
            size = ROUND_SIZE( sec->VirtualAddress, sec->SizeOfRawData, page_mask );
